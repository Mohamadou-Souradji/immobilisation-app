...
// Récupérer la liste des immobilisations
$query = $db->query("
SELECT i.id_immobilisation, i.intitule, f.id_facture
FROM immobilisation i
LEFT JOIN facture f ON i.id_immobilisation = f.id_immobilisation;
");
$listeImmobilisations = $query->fetchAll(PDO::FETCH_OBJ);
// Vérifier si l'ID de la facture à supprimer est passé en paramètre GET
if(isset($_GET['id_facture'])){
    // Supprimer la facture correspondante dans la base de données
    $query = $db->prepare('DELETE FROM facture WHERE id_facture=:id_facture');
    $query->execute(['id_facture' => $_GET['id_facture']]);
        ...
        <div id="layoutSidenav_content">
            <main>
                <div class="d-flex align-items-center justify-content-center mt-4 mb-4">
                    <a href="add_facture.php" class="btn btn-success">Ajouter <i class="fa-solid fa-plus"></i></a>
                </div>
                <div class="card mb-4">
                        ...
                        <table class="table table-success table-striped table-bordered" id="datatablesSimple">
                            ...
                            <?php foreach($listeImmobilisations as $immobilisation): ?>
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($immobilisation->id_facture): ?>
                                           
                                            <a href="facture.php?id=<?= $immobilisation->id_facture; ?>" class="btn btn-info">Voir</a>
                                            <a href="facturelist.php?id_facture=<?= $immobilisation->id_facture ?>" class="btn btn-danger btn-sm" onclick="return confirm('Êtes-vous sûr de vouloir supprimer cette facture?')"><i class="fa-solid fa-trash"></i></a>
