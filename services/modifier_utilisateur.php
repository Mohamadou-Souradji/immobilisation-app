...
// Vérifier si l'ID de l'utilisateur à modifier est défini dans l'URL
...
$query = $db->prepare("SELECT id, login,password FROM utilisateur WHERE id = :id");
$query->execute(['id' => $id]);
...
if(isset($_POST['submit'])){
    ...
    if(!empty($_POST['login']) && !empty($_POST['password'])) {
        ...
        $updateQuery = $db->prepare('UPDATE utilisateur SET login = :login, password = :password WHERE id = :id');
        $updateQuery->execute([
            'login' => $login,
            'password' => $password,
            'id' => $id,
                    ...
                    <div class="row justify-content-center">
                        <div class="col-lg-8">
                            <div class="card shadow-lg border-0 rounded-lg mt-5">
                                <div class="card-header"><h3 class="text-center font-weight-light my-3">Modifier un utilisateur</h3></div>
                                <div class="card-body">
                                    <form action="" method="post">
                                        <div class="form-floating mb-3">
                                            <input class="form-control" name="login" type="email" value="<?= $utilisateur->login ?>" required />
                                            <label for="inputEmail">Email</label>
                                        </div>
                                        <div class="form-floating mb-3">
                                            <input class="form-control" name="password" type="password"  value="<?= $utilisateur->password ?>"required />
